...
def show_conversation(identity_card_number):
    ...
    if 'Item' not in response:
        st.warning("No conversation history found for the provided Identity Card Number.")
        return

    conversations = response['Item'].get('conversation_history', '[]')

    try:
        conversations_list = json.loads(conversations)
    except json.JSONDecodeError:
        st.error("Failed to decode the conversation history.")
        return

    # st.header(f"Conversation History for ID: {identity_card_number}")
    for entry in conversations_list:
        ...
        if msg_type == 'customer':
            ...
            st.markdown(f'**Customer**: {message_text}')

        elif msg_type == 'agent':
            st.markdown(f'**Agent**: {message}')           
        else:
            st.markdown(f"*Unknown message type:* {msg_type}")
        st.markdown('---') 
